...
- (BOOL) listHomeDirectory {
    ...
    self.hud.labelText = NSLocalizedString(@"retriveing data", nil);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([file isDirectory]) {
        ...
        cell.detailTextLabel.text = [NSString stringWithFormat:NSLocalizedString(@"folder", nil)];
        //customDownloadbutton = [self makeDetailDisclosureButton:DisclosureDirType];
    }
    // if pdf doc
    else if (![file.filename hasSuffix:@".exe"]){
        cell.imageView.image = [UIImage imageNamed:[self getImageNameByFileName:file.filename]];
...
-(void)moveFileToProjectFolder:(NSString *)fileName{
    NSString* documentsPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString* sPath = [documentsPath stringByAppendingPathComponent:fileName];
    NSString* tPath =[documentsPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@/%@",self.projName, fileName]];
    [[NSFileManager defaultManager] moveItemAtPath:sPath toPath:tPath error:nil];
}
...
- (void)downloadedFile {
    ...
    [self moveFileToProjectFolder:currentFileName];
    [self.downloadProgressView setHidden:TRUE];
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"file downloaded", nil)
                                                        message:NSLocalizedString(@"msg file downloaded", nil)
                                                       delegate:nil
...
-(NSString *)getImageNameByFileName:(NSString *)fileName{
    ...
    if ([[fn lowercaseString] hasSuffix:@"jpg"] || [[fn lowercaseString] hasSuffix:@"jepg"]){
        ...
        return @"excel.png";
    }else if ([[fn lowercaseString] hasSuffix:@"exe"] || [[fn lowercaseString] hasSuffix:@"xlsx"]){
        return @"exe.png";
    }else {
        return @"Text.png";
    }
