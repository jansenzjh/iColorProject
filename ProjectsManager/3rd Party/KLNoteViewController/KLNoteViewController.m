...
-(void)updateControllerCards{
    //NSInteger *total = [self getDatabaseItems].count;
    
    NSMutableArray* navigationControllers = [[NSMutableArray alloc] initWithCapacity:([[self getDatabaseItems] count])];
    for (NSInteger count = 0; count < [[self getDatabaseItems] count]; count++) {
        UIViewController* viewController = [self noteView:self viewControllerForRowAtIndexPath:[NSIndexPath indexPathForRow:count inSection:0]];
